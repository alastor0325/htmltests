...
#   av1.mp4   - AV1
#
# Also keeps direct.mp4 / frag.mp4 (H.264) for the older progressive + MSE path.
...
# H.264 progressive + fragmented copies for the legacy direct/MSE path.
cp -f h264.mp4 direct.mp4
ffmpeg -y -hide_banner -loglevel error -i direct.mp4 \
  -c copy -movflags frag_keyframe+empty_moov+default_base_moof frag.mp4

echo "wrote: $(ls -1 h264.mp4 vp9.webm av1.mp4 direct.mp4 frag.mp4)"
